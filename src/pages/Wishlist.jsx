  ...
  if (wishlistCount === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center pt-20 px-6 font-urbanist bg-white">
        ...
        <h2 className="text-3xl font-bold text-slate-900 capitalize  mb-4">Your wishlist is empty</h2>
        <p className="text-slate-400 font-bold capitalize tracking-widest text-[10px] mb-10">Save items you love to find them easily later.</p>
        <Link to="/shop" className="px-12 py-5 bg-black text-white text-[10px] font-bold capitalize tracking-[0.3em] rounded-2xl hover:bg-red-600 transition-all shadow-xl">
          Browse Products
        </Link>
      </div>
    );
    ...
    <div className="min-h-screen bg-white pt-32 pb-20 font-urbanist">
      <div className="max-w-[1400px] mx-auto px-6 md:px-10">
        ...
        <div className="flex items-end justify-between mb-12 border-b border-gray-100 pb-8">
          <div>
            <span className="text-[10px] font-bold tracking-[0.4em] capitalize text-red-500 mb-2 block ml-1">Your Favorites</span>
          ...
          <p className="text-xs font-bold capitalize tracking-widest text-slate-400 pb-1">{wishlistCount} Saved Items</p>
        </div>
              ...
              >
                <div className="relative aspect-square bg-gray-50 rounded-[2rem] border border-gray-100 group-hover:border-red-500/20 transition-all duration-500 flex items-center justify-center p-8 overflow-hidden mb-4">
                  ...
                  <div className="absolute bottom-3 left-3 right-3 translate-y-[120%] group-hover:translate-y-0 transition-transform duration-500">
                    <button
                      onClick={() => addToCart(p)}
                      className="w-full h-10 bg-black text-white rounded-xl flex items-center justify-center gap-2 text-[9px] font-bold capitalize tracking-widest hover:bg-blue-600 transition-colors shadow-lg"
                    >
                ...
                <div className="px-1 flex-1 flex flex-col">
                  <Link to={`/product/${p.slug}`}>
                    <h3 className="text-[11px] font-bold text-slate-800 group-hover:text-blue-600 transition-colors capitalize  leading-tight line-clamp-1 mb-1">{p.name}</h3>
                  </Link>
        ...
        <div className="mt-20 pt-10 border-t border-gray-100">
          <Link to="/shop" className="inline-flex items-center gap-2 text-[10px] font-bold capitalize tracking-widest text-slate-400 hover:text-blue-600 transition-colors">
