      ...
      <div className="bg-slate-50 border-b border-slate-200">
        <div className="max-w-[1920px] mx-auto pt-32 pb-12 px-6 md:px-12">
          <div className="flex flex-col gap-6">
            <div className="flex items-center gap-2 text-[10px] font-bold capitalize tracking-widest text-slate-400">
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <aside className="hidden lg:block w-64 shrink-0 space-y-10 sticky top-32 h-fit">
            ...
            {(category || brand) && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold capitalize tracking-widest text-slate-900">Active Filters</span>
                  <button onClick={() => navigate('/shop')} className="text-[10px] font-bold text-red-500 hover:underline capitalize tracking-wider">Clear All</button>
                </div>
                <div className="flex flex-wrap gap-2">
                  {category && (
                    <button onClick={() => updateFilter('category', '')} className="flex items-center gap-2 px-3 py-1.5 bg-brand/5 text-brand rounded-lg text-[10px] font-bold capitalize border border-brand/10 hover:bg-red-50 hover:text-red-500 hover:border-red-100 transition-all group">
                      {category} <X size={10} className="group-hover:scale-110" />
                    </button>
                  )}
                  {brand && (
                    <button onClick={() => updateFilter('brand', '')} className="flex items-center gap-2 px-3 py-1.5 bg-brand/5 text-brand rounded-lg text-[10px] font-bold capitalize border border-brand/10 hover:bg-red-50 hover:text-red-500 hover:border-red-100 transition-all group">
            ...
            <div className="space-y-5">
              <span className="text-xs font-bold capitalize tracking-widest text-slate-400 block border-b border-slate-100 pb-2">Departments</span>
            ...
            {/* Brands */}
            <div className="space-y-5">
              <span className="text-xs font-bold capitalize tracking-widest text-slate-400 block border-b border-slate-100 pb-2">Brands</span>
              <div className="space-y-2">
                {brands.slice(0, 10).map(b => (
                  <label key={b.id} className="flex items-center gap-3 cursor-pointer group">
                    ...
                    <button
                      ...
                      className={`text-xs font-medium transition-colors capitalize  ${brand === b.name ? 'text-brand font-bold' : 'text-slate-600 group-hover:text-slate-900'}`}
                    >
                      {b.name}
                    </button>
                  </label>
            ...
            <div className="space-y-5">
              <span className="text-xs font-bold capitalize tracking-widest text-slate-400 block border-b border-slate-100 pb-2">Price Range</span>
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 sticky top-[80px] z-30 bg-white/90 backdrop-blur-md py-4 -mx-4 px-4 sm:mx-0 sm:px-0 lg:static lg:bg-transparent lg:py-0">
              <button
                ...
                className="lg:hidden flex items-center gap-2 px-4 py-2.5 bg-slate-900 text-white rounded-lg text-xs font-bold capitalize tracking-widest hover:bg-brand transition-all w-fit shadow-lg shadow-slate-900/10"
              >
                <SlidersHorizontal size={14} /> Filters
              </button>

              <div className="flex items-center gap-2 text-[11px] font-bold capitalize tracking-widest text-slate-400">
                <span className="text-slate-900">{total}</span> Results Found
              </div>

              <div className="flex items-center gap-4 ml-auto">
                <div className="flex items-center gap-2">
                  <span className="hidden sm:inline text-[10px] font-bold text-slate-400 capitalize tracking-widest">Sort By</span>
                  <div className="relative">
                    <select
                      value={sort}
                      onChange={(e) => updateFilter('sort', e.target.value)}
                      className="appearance-none bg-slate-50 border border-slate-200 pl-4 pr-8 py-2 rounded-lg text-xs font-bold text-slate-700 focus:outline-none focus:border-brand cursor-pointer capitalize "
                    >
            ...
            {loading ? (
              <div className="py-40 flex flex-col items-center justify-center">
                ...
                <p className="text-xs font-bold capitalize tracking-widest text-slate-400">Loading Catalog...</p>
              </div>
            ) : products.length === 0 ? (
              <div className="py-24 text-center bg-slate-50 rounded-3xl border border-dashed border-slate-200">
                ...
                <h3 className="text-lg font-bold text-slate-900 capitalize  mb-2">No Matches Found</h3>
                ...
                <button onClick={() => navigate('/shop')} className="px-6 py-3 bg-slate-900 text-white rounded-full text-xs font-bold capitalize tracking-widest hover:bg-brand transition-all shadow-lg">Clear All Filters</button>
              </div>
            ) : (
                ...
                {products.map((p, i) => (
                  ...
                  >
                    ...
                    <div className="absolute top-4 left-4 z-10 flex gap-2">
                      {p.brand_name && (
                        <span className="bg-slate-900/90 backdrop-blur-sm text-white px-2 py-1 text-[9px] font-bold capitalize tracking-wider rounded">
                          {p.brand_name}
                        </span>
                      )}
                      {i < 3 && (
                        <span className="bg-brand/90 backdrop-blur-sm text-white px-2 py-1 text-[9px] font-bold capitalize tracking-wider rounded flex items-center gap-1">
                      ...
                      {viewMode === 'grid' && (
                        <div className="absolute inset-x-4 bottom-4 translate-y-[120%] group-hover:translate-y-0 transition-transform duration-300 ease-out">
                          <button
                            ...
                            className={`w-full py-3 rounded-xl flex items-center justify-center gap-2 text-[10px] font-bold capitalize tracking-widest shadow-lg ${addedItems[p.id] ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white hover:bg-brand'}`}
                          >
                            {addedItems[p.id] ? (
                              <><Check size={14} /> Added</>
                            ) : (
                    ...
                    <div className={`flex flex-col ${viewMode === 'list' ? 'flex-1 py-2 pr-4' : 'p-5'}`}>
                      <div className="mb-1">
                        <span className="text-[10px] font-bold text-slate-400 capitalize tracking-widest">{p.category_name || 'Electronics'}</span>
                      </div>
                      <Link to={`/product/${p.slug}`} className="block mb-2">
                        <h3 className={`font-bold text-slate-900 group-hover:text-brand transition-colors capitalize  line-clamp-2 ${viewMode === 'list' ? 'text-xl' : 'text-sm leading-relaxed'}`}>
                          {p.name}
                        </h3>
                      </Link>

                      <div className="mt-auto flex items-center justify-between">
                        <p className="text-lg font-bold text-slate-900">${parseFloat(p.price).toLocaleString()}</p>

                        {viewMode === 'list' && (
                          <button
                            onClick={(e) => handleAddToCart(e, p)}
                            disabled={addedItems[p.id]}
                            className={`px-6 py-2.5 rounded-lg flex items-center justify-center gap-2 text-[10px] font-bold capitalize tracking-widest transition-all ${addedItems[p.id] ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white hover:bg-brand'}`}
                          >
            ...
            {total > 0 && products.length > 0 && (
              <div className="mt-16 border-t border-slate-100 pt-8 flex flex-col items-center gap-4">
                <p className="text-[10px] font-bold text-slate-400 capitalize tracking-widest">Showing {products.length} of {total} products</p>
          ...
          <>
            ...
            >
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-white">
                <span className="text-sm font-bold capitalize tracking-widest text-slate-900">Filters</span>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {brands.map(b => (
                      <button
                        ...
                        className={`px-4 py-2 rounded-lg text-xs font-bold capitalize border ${brand === b.name ? 'bg-brand text-white border-brand' : 'bg-slate-50 border-transparent text-slate-600'}`}
                      >
                        {b.name}
                      </button>
                    ))}
              ...
              <div className="p-6 border-t border-slate-100 bg-slate-50">
                <button
                  ...
                  className="w-full py-4 bg-slate-900 text-white rounded-xl text-xs font-bold capitalize tracking-widest hover:bg-red-500 transition-all shadow-lg"
                >
                  Reset All Filters
                </button>
              </div>
